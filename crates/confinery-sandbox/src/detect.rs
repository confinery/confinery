...
mod windows {
    ...
    pub fn detect() -> HostCapabilities {
        ...
        // profile sets `windows.container_image` -- a successful check
        // here only means `wslc` runs, not that the profile opted in.
        let wslc = crate::windows::wslc::WslcSandbox::is_available();
        features.push(if wslc {
            Feature::yes(
                "wslc",
                "`wslc version` ran successfully (WSL Containers, public preview)",
            )
        } else {
            Feature::no(
                "wslc",
                "`wslc` did not run (needs `wsl --update`, or not on PATH)",
            )
        });
